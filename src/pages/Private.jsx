...
import { 
  ...
  FiCheckCircle, FiCoffee, FiTool, FiShield
} from 'react-icons/fi';
...
// ─── THƯ VIỆN ICON PHONG PHÚ CHO ADMIN CHỌN ───
...
export default function Private() {
  ...
  const addExperience = () => setFormData({ ...formData, experiences: [...(formData.experiences || []), { title: '', company: '', date: '', description: '' }] });
      ...
      <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '75vh', backgroundColor: '#f8fafc' }}>
        <div style={{ width: '100%', maxWidth: '400px', padding: '2.5rem', borderRadius: '16px', backgroundColor: '#ffffff', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            <button type="submit" style={{ width: '100%', padding: '1rem', marginTop: '1rem', backgroundColor: '#6366F1', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer' }}>Đăng Nhập Cổng CMS</button>
          </form>
        </div>
  ...
  const cardStyle = { padding: '2rem', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0', marginBottom: '2rem' };
  const labelStyle = { fontWeight: 700, color: '#1e1b4b', display: 'block', marginBottom: '0.5rem', fontSize: '0.95rem' };

  // Helper tính số lượng stats hiện tại
  const currentStatsCount = (formData.stats || []).length;

  return (
    <section style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f1f5f9' }}>
      
      {/* ── SIDEBAR ĐIỀU HƯỚNG ── */}
      <div style={{ width: '260px', flexShrink: 0, backgroundColor: '#1e1b4b', padding: '2rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', color: '#fff' }}>
        <h3 style={{ color: '#818cf8', textTransform: 'uppercase', fontSize: '0.85rem', letterSpacing: '1.5px', marginBottom: '2rem', paddingLeft: '0.5rem' }}>Bảng Điều Khiển</h3>
        {[
      ...
      <div style={{ flex: 1, padding: '3rem 4rem', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
          <div>
            <h1 style={{ color: '#1e1b4b', margin: '0 0 0.5rem 0', fontSize: '2rem', fontWeight: 800, textTransform: 'capitalize' }}>Chỉnh Sửa: {activeTab}</h1>
            <p style={{ color: '#64748b', margin: 0 }}>Cập nhật nội dung hiển thị trực tiếp lên Website.</p>
          </div>
          <button onClick={handleSaveContent} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '1rem 2.5rem', backgroundColor: '#10b981', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 700, fontSize: '1rem', cursor: 'pointer', boxShadow: '0 10px 20px rgba(16, 185, 129, 0.2)', transition: 'transform 0.2s' }} onMouseOver={e => e.currentTarget.style.transform='translateY(-2px)'} onMouseOut={e => e.currentTarget.style.transform='translateY(0)'}>
            <FiSave size={20} /> {loading ? 'Đang đồng bộ...' : 'Lưu Thay Đổi Ngay'}
          </button>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '1.5rem', padding: '1rem', backgroundColor: '#e0e7ff', borderRadius: '8px' }}>
                ...
                <label htmlFor="openToWork" style={{ fontWeight: 700, color: '#4338ca', cursor: 'pointer', margin: 0 }}>Hiển thị nhãn dán "Đang tìm kiếm cơ hội mới (Open to work)"</label>
              </div>
            ...
            <div style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <div>
                  ...
                  <p style={{ color: '#64748b', fontSize: '0.9rem', margin: '4px 0 0 0' }}>Giới hạn tối đa 4 chỉ số để giao diện cân đối nhất ({currentStatsCount}/4).</p>
                </div>
                <button 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {(formData.stats || []).map((stat, idx) => (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 100px 100px 50px', gap: '1rem', alignItems: 'center', background: '#fff', padding: '1.2rem 1rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                    <div>
                    ...
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Tên nhãn hiển thị</label><input type="text" value={stat.label} onChange={(e) => { const s = [...formData.stats]; s[idx].label = e.target.value; setFormData({ ...formData, stats: s }); }} style={{ ...inputStyle, padding: '0.6rem', marginTop: '4px' }} placeholder="VD: Dự án, Khách hàng..."/></div>
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Con số</label><input type="number" value={stat.value} onChange={(e) => { const s = [...formData.stats]; s[idx].value = Number(e.target.value); setFormData({ ...formData, stats: s }); }} style={{ ...inputStyle, padding: '0.6rem', marginTop: '4px' }} placeholder="10" /></div>
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Hậu tố</label><input type="text" value={stat.suffix} onChange={(e) => { const s = [...formData.stats]; s[idx].suffix = e.target.value; setFormData({ ...formData, stats: s }); }} style={{ ...inputStyle, padding: '0.6rem', marginTop: '4px' }} placeholder="+, %, th"/></div>
                    <button onClick={() => setFormData({ ...formData, stats: formData.stats.filter((_, i) => i !== idx) })} style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', marginTop: '20px' }}><FiTrash2 size={20} /></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'about' && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={cardStyle}>
              <h3 style={{ color: '#1e1b4b', margin: '0 0 1.5rem 0', fontSize: '1.4rem' }}>Thông Tin Mở Đầu</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {(formData.timeline || []).map((item, idx) => (
                  <div key={idx} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '1.5rem', position: 'relative' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr', gap: '1.5rem', paddingRight: '2rem', marginBottom: '1rem' }}>
                      <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Năm/Tháng</label><input type="text" value={item.year} onChange={(e) => { const arr = [...(formData.timeline || [])]; arr[idx].year = e.target.value; setFormData({ ...formData, timeline: arr }); }} style={inputStyle} /></div>
                      <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Chức danh / Tiêu đề</label><input type="text" value={item.title} onChange={(e) => { const arr = [...(formData.timeline || [])]; arr[idx].title = e.target.value; setFormData({ ...formData, timeline: arr }); }} style={inputStyle} /></div>
                      <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Tên Công ty</label><input type="text" value={item.company || ''} onChange={(e) => { const arr = [...(formData.timeline || [])]; arr[idx].company = e.target.value; setFormData({ ...formData, timeline: arr }); }} style={inputStyle} /></div>
                    </div>
                  ...
                  <div key={idx} style={{ background: '#fff', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', gap: '1.5rem' }}>
                      <div style={{ width: '80px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {ach.logo ? <img src={ach.logo} style={{ width: '80px', height: '80px', objectFit: 'contain', border: '1px solid #e2e8f0', borderRadius: '8px' }} /> : <div style={{ width: '80px', height: '80px', background: '#f1f5f9', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FiImage color="#94a3b8"/></div>}
                      ...
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.8rem', paddingRight: '2rem' }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {(formData.education || []).map((edu, idx) => (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 40px', gap: '1rem', alignItems: 'center', background: '#fff', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Chuyên ngành</label><input type="text" value={edu.title} onChange={(e) => { const arr = [...(formData.education || [])]; arr[idx].title = e.target.value; setFormData({ ...formData, education: arr }); }} style={{ ...inputStyle, padding: '0.6rem', marginTop: '4px' }} /></div>
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Trường ĐH / Tổ chức</label><input type="text" value={edu.company} onChange={(e) => { const arr = [...(formData.education || [])]; arr[idx].company = e.target.value; setFormData({ ...formData, education: arr }); }} style={{ ...inputStyle, padding: '0.6rem', marginTop: '4px' }} /></div>
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Niên khóa</label><input type="text" value={edu.date} onChange={(e) => { const arr = [...(formData.education || [])]; arr[idx].date = e.target.value; setFormData({ ...formData, education: arr }); }} style={{ ...inputStyle, padding: '0.6rem', marginTop: '4px' }} /></div>
                    <button onClick={() => setFormData({ ...formData, education: (formData.education || []).filter((_, i) => i !== idx) })} style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', marginTop: '20px' }}><FiTrash2 size={20} /></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'projects' && (
          <div style={cardStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              ...
              <button style={{ background: '#6366f1', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 700, cursor: 'pointer' }} onClick={() => openProjectForm('add')}><FiPlus /> Soạn Tài Liệu Mới</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {(formData.projects || []).map((proj, idx) => (
                <div key={proj.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', backgroundColor: proj.isHidden ? '#f8fafc' : '#ffffff' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
                <input type="text" value={newCategoryInput} onChange={(e) => setNewCategoryInput(e.target.value)} placeholder="Nhập tên danh mục..." style={inputStyle} />
                <button type="button" onClick={handleAddCategory} style={{ background: '#10b981', color: '#fff', border: 'none', padding: '0 2rem', borderRadius: '8px', cursor: 'pointer', fontWeight: 700 }}>Thêm</button>
              </div>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {(formData.posts || []).map((post, idx) => (
                  <div key={post.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', backgroundColor: post.isHidden ? '#f8fafc' : '#ffffff' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ padding: '1.5rem', backgroundColor: '#eff6ff', borderRadius: '8px', border: '1px dashed #6366f1' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <input type="text" value={formData.cvUrl || ''} readOnly style={inputStyle} />
            ...
            <div style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                ...
                <button style={{ background: '#6366f1', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontWeight: 700 }} onClick={addSkill}><FiPlus /> Thêm Nhóm</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {(formData.skills || []).map((skill, idx) => (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 40px', gap: '1.5rem', alignItems: 'center', background: '#fff', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                    <div><label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#64748b' }}>Tên nhóm</label><input type="text" value={skill.title} onChange={(e) => updateSkill(idx, 'title', e.target.value)} placeholder="VD: Công cụ" style={{ ...inputStyle, marginTop: '4px' }}/></div>
                    <div>
                    ...
                    <button type="button" onClick={() => removeSkill(idx)} style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', marginTop: '20px' }}><FiTrash2 size={20}/></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'global' && (
          <div style={cardStyle}>
            <h3 style={{ margin: '0 0 2rem 0', color: '#1e1b4b', fontSize: '1.4rem' }}>Cấu Hình Hệ Thống Chung</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', padding: '1.5rem', backgroundColor: '#f1f5f9', borderRadius: '12px' }}>
