...
function Resume() {
  ...
  return (
    <section className="page resume-page">
      ...
      <PageHero 
        title={content?.pageTitle || 'Hồ Sơ Năng Lực'} 
        subtitle={content?.subtitle || 'Chi tiết kỹ năng và kinh nghiệm'} 
        bgImage={content?.coverUrl || ''} 
      />

      <div style={{ padding: '0 1.5rem 4rem 1.5rem', maxWidth: '900px', margin: '0 auto' }}>
        <div className="resume-box reveal-section" style={{ backgroundColor: 'var(--bg-white)', padding: 'clamp(1.5rem, 5vw, 4rem)', borderRadius: '16px', boxShadow: '0 4px 30px rgba(0, 0, 0, 0.03)', border: '1px solid var(--border-color)' }}>
          ...
          {(displayMode === 'both' || displayMode === 'dynamic_only') && (
            <>
              <section className="reveal-section" style={{ marginBottom: '3rem' }}>
              ...
              {experiences.length > 0 && (
                <section className="reveal-section" style={{ marginBottom: '3rem' }}>
                  ...
                  {experiences.map((exp, idx) => (
                    <div key={idx} style={{ marginBottom: '2rem', paddingLeft: '1rem', borderLeft: '2px solid var(--primary-color)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', fontWeight: 600, marginBottom: '0.4rem', flexWrap: 'wrap', gap: '0.5rem' }}>
              ...
              {skills.length > 0 && (
                <section className="reveal-section" style={{ marginBottom: '3rem' }}>
                  <h3 style={{ fontSize: 'var(--font-h4)', color: 'var(--text-main)', fontWeight: 800, paddingBottom: '0.5rem', marginBottom: '1.5rem', textTransform: 'uppercase', letterSpacing: '1px' }}>Kỹ Năng Nền Tảng</h3>
                  <div className="mobile-grid-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: '1rem' }}>
                    {skills.map((s, i) => (
                      <div key={i} style={{ padding: '1.5rem', backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '8px' }}>
          ...
          {(displayMode === 'both' || displayMode === 'pdf_only') && cvUrl !== '#' && cvUrl && (
            <section className="reveal-section" style={{ marginBottom: '3rem', marginTop: '2rem' }}>
          ...
          <div className="reveal-section" style={{ textAlign: 'center', paddingTop: '2.5rem', borderTop: '1px solid var(--border-color)' }}>
