...
function Contact() {
  ...
  return (
    <section className="page contact-page">
      ...
      <PageHero 
        title={globalContent?.contactTitle || 'Liên Hệ Trực Tiếp'} 
        subtitle={globalContent?.contactSubtitle || 'Để lại thông tin kết nối công việc hoặc dự án hợp tác'} 
        bgImage={globalContent?.contactCoverUrl || ''} 
      />

      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem 4rem 1.5rem' }}>
        <div className="mobile-grid-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 350px), 1fr))', gap: '3rem' }}>
          
          {/* THÔNG TIN LIÊN HỆ */}
          <div className="contact-box reveal-section" style={{ backgroundColor: 'var(--bg-white)', padding: '3rem', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          ...
          {/* FORM LIÊN HỆ */}
          <form className="contact-box reveal-section" onSubmit={handleSubmit} style={{ backgroundColor: 'var(--bg-white)', padding: '3rem', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
